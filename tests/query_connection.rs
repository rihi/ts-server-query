...
use ts_server_query::{Command, query_connection_parts};
...
async fn queries_response_from_split_stream() -> Result<(), Box<dyn Error>> {
    ...
    let server = tokio::spawn(async move {
        ...
        server_stream.write_all(b"error id=0 msg=ok\n\r").await?;
...
#[tokio::test]
async fn sequences_events_and_responses_in_receive_order() -> Result<(), Box<dyn Error>> {
    let (client_stream, mut server_stream) = duplex(1024);
    let (reader, writer) = tokio::io::split(client_stream);
    let (client, connection) = query_connection_parts(reader, writer);
    let connection = tokio::spawn(connection);
    ...
    let server = tokio::spawn(async move {
        ...
        server_stream
            .write_all(
                b"notifycliententerview clid=7\n\r\
                  error id=0 msg=ok\n\r\
                  notifyclientleftview clid=7\n\r",
            )
            .await?;
    ...
    let response = timeout(TEST_TIMEOUT, client.send(Command::new("version")?)).await??;
    let first_event = timeout(TEST_TIMEOUT, events.recv()).await??;
    let second_event = timeout(TEST_TIMEOUT, events.recv()).await??;
